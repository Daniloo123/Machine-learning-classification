...
def extract_from_json_as_np_array(key, json_data):
    """ Hulpfunctie om data uit de JSON te halen en om te zetten naar een NumPy-array voor sklearn """
    data_as_array = []
    for p in json_data:
        data_as_array.append(p[key])
    return np.array(data_as_array)

STUDENTNUMMER = "0955071" # TODO: pas aan naar je eigen studentnummer

assert STUDENTNUMMER != "1234567", "Verander 1234567 naar je eigen studentnummer"
...
# KMeans clustering
kmeans = KMeans(n_clusters=3)  # aantal clusters instellen
kmeans.fit(X)
labels = kmeans.predict(X)

# Plot de punten met kleuren
plt.scatter(X[:, 0], X[:, 1], c=labels, cmap='Set1', marker='.')
...
# Gesuperviseerd leren - Train Model
...
nauwkeurigheid = accuracy_score(Y, Y_predict)
print("Classificatienauwkeurigheid (training):", nauwkeurigheid)
...
# Stuur voorspellingen naar de server
classification_test = data.classification_test(Z.tolist())
print("Classificatienauwkeurigheid (test):", classification_test)
"""
...
STUDENTNUMMER = "0955071" # TODO: aanpassen aan je eigen studentnummer
